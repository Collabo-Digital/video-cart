...
//   //       // Send to backend
//   //       fetch('https://your-backend.com/api/analytics/add-to-cart', {
//   //         method: 'POST',
...
//           console.warn('[Video Cart Pixel] Conversion skipped: set API base URL and Account ID in pixel settings.');
//           return;
//         }
//         console.log('[Video Cart Pixel] Conversion:', videoItems);
//         const url = `${process.env.SHOPIFY_APP_URL}/api/v1/analytics/conversion`;
//         fetch(url, {
//           method: 'POST',
//           headers: { 'Content-Type': 'application/json' },
//           body: JSON.stringify({ shop: window.Shopify?.shop || 'unknown', items: videoItems }),
//         }).catch((err) => console.error('[Video Cart Pixel] Conversion send error:', err));
//       } catch (e) {
  ...
  analytics.subscribe("checkout_completed", async (event) => {
    ...
    if (videoItems.length > 0) {
      try {
        ...
        const backendUrl = settings?.apiBaseUrl || process.env.SHOPIFY_APP_URL || 'https://your-actual-backend-url.com';
